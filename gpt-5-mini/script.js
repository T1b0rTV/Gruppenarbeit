...
function clearBoard(keepPlaced = false) {
        ...
        for (let r = 0; r < ROWS; r++) {
            for (let c = 0; c < COLS; c++) {
                if (board[r][c]) {
                    ...
                    if (cell) cell.classList.add('filled', board[r][c]);
                }
            }
        }
    }
    ...
    for (let r = 0; r < piece.shape.length; r++) {
        for (let c = 0; c < piece.shape[r].length; c++) {
            if (piece.shape[r][c]) {
                ...
                if (newRow >= ROWS || newCol < 0 || newCol >= COLS || (board[newRow] && board[newRow][newCol])) {
                    return false;
                }
            }
        }
